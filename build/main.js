	...
	async queryData() {
		...
		try {
			...
			if (Array.isArray(this.trackerId) && this.trackerId.length > 0) {
				...
				this.log.warn(
					'\u26A0\uFE0F No tracker IDs returned from API \u2014 check device configuration or credentials.',
				);
				this.trackerId = [];
			}
		} catch (error) {
		...
		this.apiManager
			...
			.then(async positions => {
				for (const position of positions) {
					console.log(
						`[${this.customerId}] Position ID: ${position.id}, Latitude: ${Math.round(position.lat * 1e4) / 1e4}, Longitude: ${Math.round(position.lng * 1e4) / 1e4}`,
					);
					this.log.debug(
						`Position ID: ${position.id}, Latitude: ${Math.round(position.lat * 1e4) / 1e4}, Longitude: ${Math.round(position.lng * 1e4) / 1e4}`,
					);
